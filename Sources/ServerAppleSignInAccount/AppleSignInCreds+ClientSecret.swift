...
extension AppleSignInCreds {
    ...
    func createClientSecret() throws -> String {
        // This is sensitive to the string format of the incoming privateKey. e.g., It botches things up when I load it from a plist.
        let privateKey = config.privateKey.replacingOccurrences(of: "\\n", with: "\n")

        guard let privateKeyData = privateKey.data(using: .utf8) else {
            throw AppleSignInCredsError.noPrivateKeyData
        }
        ...
        if #available(OSX 10.13, *) {
           ...
           throw AppleSignInCredsError.couldNotSignJWT
        }
